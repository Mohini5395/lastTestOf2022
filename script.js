let peopleData = `[
    ...
    let data = JSON.parse(peopleData)
    console.log(typeof data);
    
    let searchText = document.getElementById("inputField")
    let searchBtn = document.getElementById("search");
    let alphaAscending = document.getElementById("aToz");
    let alphaDecending = document.getElementById("zToA");
    let sortMarks = document.getElementById("marks");
    let sortPassing = document.getElementById("passing");
    let sortClass = document.getElementById("classing");
    let sortGender = document.getElementById("gender");
    
    
      sortMarks.addEventListener("click", sortingAscMarks)
      sortClass.addEventListener("click", sortingAscClass)
      sortPassing.addEventListener("click", sortingBypassingStatus) 
      searchBtn.addEventListener("click", searchWhatYouWant)
      alphaAscending.addEventListener("click", sortingAscNames)
      alphaDecending.addEventListener("click", sortingDesNames)
      sortGender.addEventListener("click", searchByGender, searchbyMaleGender,)
    ...
    wholeData.append(lItem);
    })
    }
    showDataOnPage(data);
    
     function sortingAscNames(){
        let sortedNames = data.sort((a, b)=>{
            if(a.first_name.toLowerCase()< b.first_name.toLowerCase()) return -1;
            else if(a.first_name.toLowerCase()> b.first_name.toLowerCase()) return 1;
            else return 0;
        })
        showDataOnPage(sortedNames)
    }
    
     function sortingDesNames(){
        let decendingSorted = data.sort((a, b)=> {
            if(a.first_name.toLowerCase()< b.first_name.toLowerCase()) return 1;
            else if(a.first_name.toLowerCase()> b.first_name.toLowerCase()) return -1;
            else return 0;
        }) 
        showDataOnPage(decendingSorted) 
     }
    
     function sortingAscMarks(){
         let finalSort = data.sort((a , b)=> {
             if(a.marks < b.marks) return -1;
             else if(a.marks > b.marks) return 1;
             else return 0;
         })
        showDataOnPage(finalSort);
    }
    
     function sortingBypassingStatus(){
    let passedStudents = data.filter((item)=> {
        return item.passing
    })
    showDataOnPage(passedStudents);
    }
    
     function sortingAscClass(){
        let finalClass = data.sort((a , b)=> {
            if(a.class < b.class) return -1;
            else if(a.class > b.class) return 1;
            else return 0;
     })
     showDataOnPage(finalClass);
    }
    
    function searchWhatYouWant(){
    ...
    function searchbyMaleGender(){
    let secondResult = data.filter((item)=>{
        return item.gender.toLowerCase() === "male"
    })
    showDataOnPage(secondResult);
    table2.append(secondResult);
    }